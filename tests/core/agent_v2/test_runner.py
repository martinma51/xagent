...
async def test_runner_inject_user_message_with_files_dispatches_trace_callback(
    ...
    """End-to-end coverage of the v2-continuation chip path: a websocket-style
    ...
    # Seed the registry with a fresh run so post_user_message has a live
    ...
    # (a) files survive on the Message metadata for checkpoint round-trip.
    assert new_user_message.metadata.get("files") == files

    # (b) the trace callback emitted a user_message event with files surfaced
    # at the top level for the frontend chip renderer.
    user_message_events = [
        event
