...
TRACE_WATERMARK_KEY = "_v2_user_message_trace_watermark"
...
class TraceEventCallback:
    ...
    def _files_from_context(self, context: Any) -> list[dict[str, Any]]:
        """Extract uploaded-file metadata from the v2 execution context.

        The websocket handler wraps the raw context dict — including
        ``file_info`` — inside ``metadata["request_context"]`` when starting
        a v2 run (see ``AgentV2ExecutionAdapter.execute``). Project that into
        the minimal shape the frontend FileAttachment chip needs, stripping
